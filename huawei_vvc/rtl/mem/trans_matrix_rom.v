//===========================================================================
// trans_matrix_rom.v - DCT2/DST7/DCT8 transform matrix ROM
// Auto-generated by gen_trans_matrix_rom.py
// Generated: 2026-06-08 21:31:52
...
//   RTL ROM stores Python table DIRECTLY (no transpose)
//   RTL reads: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row]
//            = matrix[in_idx][out_idx]
// Coefficient range: [-91, 91] (signed 8-bit)
// Coverage: DCT2(4/8/16/32/64), DST7(4/8/16/32), DCT8(4/8/16/32)
//===========================================================================

/* verilator lint_off UNUSEDSIGNAL */

module trans_matrix_rom (
    input  wire        clk,
    input  wire [1:0]  tr_type,     // 0=DCT2, 1=DST7, 2=DCT8
    input  wire [6:0]  length,      // 4/8/16/32/64
    input  wire [6:0]  row_addr,    // output index (out_idx)
    input  wire [6:0]  col_addr,    // input index (in_idx)
    output wire [7:0]  rd_data      // signed 8-bit matrix coefficient
);
    ...
    //===========================================================================
    // Matrix storage: [out][in] = TRANS_MATRIX layout (no transpose)
    //===========================================================================

    // DCT2_4: 4x4, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DCT2_4 [0:3][0:3] = '{
        '{  64,   64,   64,   64},
        '{  83,   36,  -36,  -83},
        '{  64,  -64,  -64,   64},
        '{  36,  -83,   83,  -36}
    ...
    // DCT2_8: 8x8, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DCT2_8 [0:7][0:7] = '{
    ...
    // DCT2_16: 16x16, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DCT2_16 [0:15][0:15] = '{
    ...
    // DCT2_32: 32x32, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DCT2_32 [0:31][0:31] = '{
    ...
    // DCT2_64: 64x64, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DCT2_64 [0:63][0:63] = '{
    ...
    // DST7_4: 4x4, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DST7_4 [0:3][0:3] = '{
        '{  29,   55,   74,   84},
        '{  74,   74,    0,  -74},
        '{  84,  -29,  -74,   55},
        '{  55,  -84,   74,  -29}
    ...
    // DCT8_4: 4x4, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DCT8_4 [0:3][0:3] = '{
        '{  84,   74,   55,   29},
        '{  74,    0,  -74,  -74},
        '{  55,  -74,  -29,   84},
        '{  29,  -74,   84,  -55}
    ...
    // DST7_8: 8x8, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DST7_8 [0:7][0:7] = '{
    ...
    // DCT8_8: 8x8, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DCT8_8 [0:7][0:7] = '{
    ...
    // DST7_16: 16x16, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DST7_16 [0:15][0:15] = '{
    ...
    // DCT8_16: 16x16, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DCT8_16 [0:15][0:15] = '{
    ...
    // DST7_32: 32x32, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DST7_32 [0:31][0:31] = '{
    ...
    // DCT8_32: 32x32, stored as [out][in] = TRANS_MATRIX layout
    // Read: ROM[col_addr][row_addr] = TRANS_MATRIX[col][row] = matrix[in][out]
    localparam signed [7:0] DCT8_32 [0:31][0:31] = '{
    ...
    //===========================================================================
    // Index wires - size-specific bit widths to avoid WIDTHTRUNC warnings
    //===========================================================================

    wire [1:0] row_idx_4 = row_addr[1:0];
    wire [1:0] col_idx_4 = col_addr[1:0];
    wire [2:0] row_idx_8 = row_addr[2:0];
    wire [2:0] col_idx_8 = col_addr[2:0];
    wire [3:0] row_idx_16 = row_addr[3:0];
    wire [3:0] col_idx_16 = col_addr[3:0];
    wire [4:0] row_idx_32 = row_addr[4:0];
    wire [4:0] col_idx_32 = col_addr[4:0];
    wire [5:0] row_idx_64 = row_addr[5:0];
    wire [5:0] col_idx_64 = col_addr[5:0];

    //===========================================================================
    // Read logic - combinational
    // Read: ROM[col_idx][row_idx] = TRANS_MATRIX[col][row] = matrix[in][out]
    //===========================================================================

    reg signed [7:0] read_matrix;
    ...
    always @(*) begin
        ...
        case (tr_type)
            TR_DCT2: begin
                case (length)
                    7'd4: read_matrix = DCT2_4[col_idx_4][row_idx_4];
                    7'd8: read_matrix = DCT2_8[col_idx_8][row_idx_8];
                    7'd16: read_matrix = DCT2_16[col_idx_16][row_idx_16];
                    7'd32: read_matrix = DCT2_32[col_idx_32][row_idx_32];
                    7'd64: read_matrix = DCT2_64[col_idx_64][row_idx_64];
                    default: read_matrix = 8'sd0;
                endcase
            end
            TR_DST7: begin
                case (length)
                    7'd4: read_matrix = DST7_4[col_idx_4][row_idx_4];
                    7'd8: read_matrix = DST7_8[col_idx_8][row_idx_8];
                    7'd16: read_matrix = DST7_16[col_idx_16][row_idx_16];
                    7'd32: read_matrix = DST7_32[col_idx_32][row_idx_32];
                    default: read_matrix = 8'sd0;
                endcase
            end
            TR_DCT8: begin
                case (length)
                    7'd4: read_matrix = DCT8_4[col_idx_4][row_idx_4];
                    7'd8: read_matrix = DCT8_8[col_idx_8][row_idx_8];
                    7'd16: read_matrix = DCT8_16[col_idx_16][row_idx_16];
                    7'd32: read_matrix = DCT8_32[col_idx_32][row_idx_32];
                    default: read_matrix = 8'sd0;
                endcase
            end
            default: read_matrix = 8'sd0;
