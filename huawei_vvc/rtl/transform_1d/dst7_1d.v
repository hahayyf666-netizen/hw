//===========================================================================
// dst7_1d.v - DST7 1D反变换 (优化版)
// 功能: 支持4/8/16/32点DST7反变换，一拍4点并行输出
// 矩阵方向: out[out_idx] = sum(matrix[in_idx][out_idx] * coeffs[in_idx])
//===========================================================================
...
module dst7_1d (
    ...
    input  wire        start,
    output reg         done,
    output reg         out_valid,      // 4点输出有效
    input  wire [6:0]  length,         // 4/8/16/32
    input  wire [4:0]  shift,          // 右移位数
    ...
    // 4点并行输出
    output reg  [31:0] result_out0,        // 第0点
    output reg  [31:0] result_out1,        // 第1点
    output reg  [31:0] result_out2,        // 第2点
    output reg  [31:0] result_out3         // 第3点
);
    ...
    trans_matrix_rom u_matrix_rom_0 (
        .clk(clk), .tr_type(2'd1), .length(length),
        .row_addr(row_base), .col_addr(col_cnt), .rd_data(matrix_data[0])
    );
    trans_matrix_rom u_matrix_rom_1 (
        .clk(clk), .tr_type(2'd1), .length(length),
        .row_addr(row_base + 7'd1), .col_addr(col_cnt), .rd_data(matrix_data[1])
    );
    trans_matrix_rom u_matrix_rom_2 (
        .clk(clk), .tr_type(2'd1), .length(length),
        .row_addr(row_base + 7'd2), .col_addr(col_cnt), .rd_data(matrix_data[2])
    );
    trans_matrix_rom u_matrix_rom_3 (
        .clk(clk), .tr_type(2'd1), .length(length),
        .row_addr(row_base + 7'd3), .col_addr(col_cnt), .rd_data(matrix_data[3])
    );
    ...
    generate
        for (i = 0; i < 4; i = i + 1) begin : clip_shift_gen
            ...
            clip u_clip (
                .in_data   (shifted_result_reg[i]),
                .clip_en   (clip_en),
                .clip_bits (clip_bits),
                .out_data  (clipped_result[i])
            );
        end
    endgenerate
